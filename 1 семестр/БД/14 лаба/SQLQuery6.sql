...
USE BANK
GO
CREATE TRIGGER BANK_AFTER_DEL1 ON БАНК AFTER DELETE
AS PRINT '1';
 RETURN;
GO
CREATE TRIGGER BANK_AFTER_DEL2 ON БАНК AFTER DELETE
AS PRINT '2';
 RETURN;
GO
CREATE TRIGGER BANK_AFTER_DEL3 ON БАНК AFTER DELETE
AS PRINT '3';
 RETURN;
GO

DELETE БАНК WHERE ID=5


SELECT T.NAME, E.TYPE_DESC
  FROM SYS.TRIGGERS  T JOIN  SYS.TRIGGER_EVENTS E  ON T.OBJECT_ID = E.OBJECT_ID
  WHERE OBJECT_NAME(T.PARENT_ID)='БАНК' AND E.TYPE_DESC = 'DELETE' ;
